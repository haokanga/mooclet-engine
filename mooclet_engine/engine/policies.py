...
def choose_policy_group(variables, context):
    ...
    if "learner" not in context:
        ...
        if Value.objects.filter(variable=grp_var, learner=context["learner"]).exists():
            ...
            version = user_policy.run_policy(
                {"mooclet": context["mooclet"], "learner": context["learner"], "used_choose_group": True})
...
def thompson_sampling(variables, context):
    ...
    for version in versions:
        if "used_choose_group" in context and context["used_choose_group"] == True:
            student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                context={'version': version, 'mooclet': context['mooclet'], 'policy': 'thompson_sampling'})
        else:
            student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                context={'version': version, 'mooclet': context['mooclet']})
        if student_ratings:
...
def sample_without_replacement(variables, context):
    ...
    if policy_parameters:
        ...
        if policy_parameters["type"] == "per-user" and context["learner"]:
            ...
            previous_versions = Version.objects.filter(value__variable__name="version",
                                                       value__learner=context["learner"], mooclet=mooclet).all()
        ...
        if 'variables' in policy_parameters and previous_versions:
            ...
            values = Value.objects.filter(version__in=previous_versions,
                                          variable__name__in=variables.keys()).select_related('variable',
                                                                                              'version').all()
            value_list = []
        ...
        if conditions:
            ...
            for condition in conditions:
                correct_versions = correct_versions.all().filter(
                    Q(value__variable__name=condition, value__text=conditions[condition]))
        ...
        if not previous_versions:
            previous_versions = Version.objects.filter(value__variable__name="version",
                                                       mooclet=mooclet).all()  # Value.objects.filter(mooclet=mooclet, variable__name="version").values_list("version", flat=True)
        if previous_versions:
            version = sample_no_replacement(all_versions, previous_versions)
        else:
            version = choice(all_versions)
    ...
    if policy_parameters:
        ...
        if policy_parameters["type"] == "per-user" and context["learner"]:
            ...
            if bool(previous_versions_user):
                ...
                previous_version_factors = Value.objects.filter(variable__name__in=factor_names,
                                                                version__in=previous_versions_user)

                new_factors = all_versions.exclude(value__variable__name__in=factor_names,
                                                   value__text__in=previous_version_factors.values_list('text',
                                                                                                        flat=True)).all()
...
def posteriors(y, X, m_pre, V_pre, a1_pre, a2_pre):
    ...
    m_post = np.dot(np.linalg.inv(np.add(np.linalg.inv(V_pre), np.dot(Xtranspose, X))),
                    np.add(np.dot(np.linalg.inv(V_pre), m_pre), np.dot(Xtranspose, y)))
...
def thompson_sampling_postdiff(variables, context):
    ...
    for version in versions:
        ...
        if "used_choose_group" in context and context["used_choose_group"] == True:
            student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                context={'version': version, 'mooclet': context['mooclet'], 'policy': 'thompson_sampling_postdiff'})
        else:
            student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                context={'version': version, 'mooclet': context['mooclet']})
        if student_ratings:
...
def thompson_sampling_uniform_start(variables, context):
    ...
    n_enrolled = Value.objects.filter(variable__name="version", mooclet=context["mooclet"],
                                      policy__name="thompson_sampling_uniform_start").count()
    ...
    if n_enrolled <= uniform_threshold:
        ...
        for version in versions:
            if "used_choose_group" in context and context["used_choose_group"] == True:
                student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                    context={'version': version, 'mooclet': context['mooclet'],
                             'policy': 'thompson_sampling_uniform_start'})
            else:
                student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                    context={'version': version, 'mooclet': context['mooclet']})
            if student_ratings:
...
def thompson_sampling_batched(variables, context):
    ...
    current_enrolled = Value.objects.filter(variable__name="version", mooclet=context["mooclet"],
                                            policy__name="thompson_sampling_batched").count()
    ...
    if "current_posteriors" not in policy_parameters or current_enrolled % batch_size == 0:
        ...
        for version in versions:
            if "used_choose_group" in context and context["used_choose_group"] == True:
                student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                    context={'version': version, 'mooclet': context['mooclet'], 'policy': 'thompson_sampling_batched'})
            else:
                student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                    context={'version': version, 'mooclet': context['mooclet']})
            if student_ratings:
    ...
    for version in current_posteriors:
        ...
        version_beta = beta(current_posteriors[version]["successes"] + prior_success,
                            current_posteriors[version]["failures"] + prior_failure)
...
def ts_configurable(variables, context):
    ...
    outcome_variable_name = policy_parameters['outcome_variable_name']
    # max value of version rating, from qualtrics
    min_rating, max_rating = policy_parameters["min_rating"] if "min_rating" in policy_parameters else 0, \
                             policy_parameters['max_rating']
    ...
    if "uniform_threshold" in policy_parameters:
        uniform_threshold = policy_parameters["uniform_threshold"]
    current_enrolled = Value.objects.filter(variable__name="version", mooclet=context["mooclet"],
                                            policy__name="ts_configurable").count()
    ...
    if "current_posteriors" not in policy_parameters or current_enrolled % batch_size == 0:
        ...
        for version in versions:
            if "used_choose_group" in context and context["used_choose_group"] == True:
                student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                    context={'version': version, 'mooclet': context['mooclet'], 'policy': 'ts_configurable'})
            else:
                student_ratings = Variable.objects.get(name=outcome_variable_name).get_data(
                    context={'version': version, 'mooclet': context['mooclet']})
            if student_ratings:
